...
function renderChamados() {
    ...
    main.innerHTML = `
    <div class="top-bar"><h1>📋 Chamados — ${sanitizar(nomeDepto)} (${f.length})</h1></div>
...
function renderLinhasChamados(l) {
    ...
    return l.sort((a, b) => toDate(b.data_abertura) - toDate(a.data_abertura)).map(c => {
        let ac = `<button class="btn btn-sm btn-primary" onclick="event.stopPropagation();avancarStatus('${c.fid}')">▶</button>`;
...
async function avancarStatus(id) {
    try {
        ...
        if (!doc.exists) { toast('Não encontrado', 'error'); return; }
        const d = doc.data();
        const fluxo = { 'A Fazer': 'Em Andamento', 'Em Andamento': 'Pendente', 'Pendente': 'Concluído', 'Concluído': 'A Fazer' };
        const ns = fluxo[d.status || 'A Fazer'] || 'A Fazer';
        const upd = { status: ns, data_atualizacao: firebase.firestore.Timestamp.now() };
        if (ns === 'Em Andamento' || ns === 'Concluído') upd.tecnico = usuarioLogado.nome;
        const timeline = d.timeline || [];
        timeline.push({ data: new Date().toISOString(), status: ns, acao: `Status alterado para "${ns}" por ${usuarioLogado.nome}` });
        upd.timeline = timeline;
        await docRef.update(upd);
        toast('✅ Atualizado para: ' + ns, 'success');
        
        if ((ns === 'Em Andamento' || ns === 'Concluído') && d.contato) {
            const num = d.contato.replace(/\D/g, '');
            if (num.length >= 10 && confirm('📱 Notificar solicitante via WhatsApp?')) {
                const msg = ns === 'Em Andamento' ? 
                    `✅ *HelpHosp*\n\nSeu chamado *${d.protocolo}* foi iniciado!\n\n📝 ${d.titulo}\n🔧 Técnico: ${usuarioLogado.nome}\n📍 ${d.setor}` :
                    `🎉 *HelpHosp*\n\nSeu chamado *${d.protocolo}* foi concluído!\n\n📝 ${d.titulo}\n🔧 Técnico: ${usuarioLogado.nome}\n📍 ${d.setor}`;
...
function verDetalhes(id) {
    ...
    const cont = `
    <div style="background:linear-gradient(135deg,var(--primary),#1a3a5c);color:white;padding:20px;border-radius:12px;margin-bottom:16px;">
        <h3>${sanitizar(c.titulo||'Sem título')}</h3><p style="margin-top:8px;">📋 Protocolo: ${sanitizar(c.protocolo||'—')}</p><p>📅 Aberto em: ${fmtData(c.data_abertura)}</p></div>
    <div style="background:#F0FDF4;padding:16px;border-radius:10px;border:2px solid #86EFAC;margin-bottom:14px;"><h4 style="color:#166534;">👤 Solicitante</h4>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <div style="background:white;padding:10px;border-radius:6px;"><strong>Nome:</strong> ${sanitizar(c.solicitante||'—')}</div>
            <div style="background:white;padding:10px;border-radius:6px;"><strong>📱 Contato:</strong> ${contatoInfo}</div>
            <div style="background:white;padding:10px;border-radius:6px;"><strong>📍 Setor:</strong> ${sanitizar(c.setor||'—')}</div>
            <div style="background:white;padding:10px;border-radius:6px;"><strong>🏢 Depto:</strong> ${sanitizar((c.departamento||'TI')==='TI'?'🖥️ TI':'🔧 Manutenção')}</div></div></div>
    <div style="background:#EFF6FF;padding:16px;border-radius:10px;border:2px solid #93C5FD;margin-bottom:14px;"><h4 style="color:#1E40AF;">📋 Chamado</h4>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <div style="background:white;padding:10px;border-radius:6px;"><strong>⚡ Prioridade:</strong> ${sanitizar(c.prioridade||'—')}</div>
            <div style="background:white;padding:10px;border-radius:6px;"><strong>📊 Status:</strong> ${sanitizar(c.status||'—')}</div>
            <div style="background:white;padding:10px;border-radius:6px;"><strong>📋 Categoria:</strong> ${sanitizar(c.categoria||'—')}</div>
            <div style="background:white;padding:10px;border-radius:6px;"><strong>🔧 Técnico:</strong> ${sanitizar(c.tecnico||'Não atribuído')}</div>
            ${c.executante?`<div style="background:white;padding:10px;border-radius:6px;"><strong>👷 Executante:</strong> ${sanitizar(c.executante)}</div>`:''}
            <div style="background:white;padding:10px;border-radius:6px;"><strong>⏱️ SLA:</strong> <span class="sla-alert ${getSLAStatus(c)}">${getSLATexto(c)}</span></div></div></div>
    ...
    <div style="display:flex;gap:6px;justify-content:flex-end;">
        <button class="btn btn-primary btn-sm" onclick="avancarStatus('${c.fid}');this.closest('.modal-overlay').remove();">▶ Avançar</button>
        ${depto==='MANUTENCAO'?`<button class="btn btn-outline btn-sm" onclick="atribuirExecutante('${c.fid}');this.closest('.modal-overlay').remove();">👷</button>`:''}
        <button class="btn btn-outline btn-sm" onclick="abrirComentarios('${c.fid}');this.closest('.modal-overlay').remove();">💬</button>
        ${getPerms().isAdmin?`<button class="btn btn-danger btn-sm" onclick="excluirChamado('${c.fid}');this.closest('.modal-overlay').remove();">🗑️</button>`:''}</div>`;
...
function verDetalhesGestHosp(c) {
    ...
    <div style="display:flex;gap:6px;justify-content:flex-end;"><button class="btn btn-primary btn-sm" onclick="avancarStatus('${c.fid}');this.closest('.modal-overlay').remove();">▶</button>${getPerms().isAdmin?`<button class="btn btn-danger btn-sm" onclick="excluirChamado('${c.fid}');this.closest('.modal-overlay').remove();">🗑️</button>`:''}</div>`;
...
function abrirComentarios(id) {
    ...
    abrirModal(`<div class="modal-header"><h3>💬 Comentários</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()">&times;</button></div>
        <p>${sanitizar(c.protocolo||'')}</p><div style="max-height:200px;overflow-y:auto;margin:10px 0;">${cs||'<p>Nenhum</p>'}</div>
        <div style="display:flex;gap:6px;"><textarea id="textoComentario" placeholder="Digite..." style="flex:1;padding:8px;border:1.5px solid var(--border);border-radius:6px;font-size:12px;resize:none;height:38px;"></textarea><button class="btn btn-primary btn-sm" onclick="salvarComentario()">Enviar</button></div>`);
}
